...
export const Navbar: React.FC<NavbarProps> = ({ onScheduleClick, onAboutClick }) => {
    ...
    <nav className={`fixed w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-white/90 backdrop-blur-md shadow-sm py-4' : 'bg-transparent py-6'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center">
        ...
        <div className="hidden md:flex items-center space-x-8">
          ...
          <a href="#about" onClick={handleAboutClick} className="text-gray-600 hover:text-gray-900 font-medium transition-colors">About</a>
          <button 
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden absolute top-full left-0 w-full bg-white border-b border-gray-100 p-4 flex flex-col space-y-4 shadow-xl">
          ...
          <a href="#about" className="text-gray-600 font-medium" onClick={handleAboutClick}>About</a>
          <button 
            onClick={() => {
              setIsMobileMenuOpen(false);
              onScheduleClick();
